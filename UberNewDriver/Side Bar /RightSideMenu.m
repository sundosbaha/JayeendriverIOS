...
@property (strong, nonatomic) IBOutlet UIImageView *bgNameImageView;
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [self.userProfileImageView applyRoundedCornersFullWithColor:[UIColor whiteColor]];
    NSLog(@"Arrayuser-%@,",arrUser);
    [self.userProfileImageView downloadFromURL:[arrUser valueForKey:PREF_USER_PICTURE] withPlaceholder:nil];
    self.nameLbl.font=[UberStyleGuide fontRegularBold:15];
    self.nameLbl.text=[NSString stringWithFormat:@"%@ %@",[arrUser valueForKey:@"first_name"],[arrUser valueForKey:@"last_name"]];
    NSLog(@"Arrayuser-%@,",self.nameLbl.text);
    ...
    // changed by natarajan commented unused loop
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.lblName setTextAlignment:NSTextAlignmentRight];
                                                   ...
                                                   [afn getDataFromPath:FILE_LOGOUT withParamData:dictparam withBlock:^(id response, NSError *error)
                                                    {
                                                        ...
                                                        if (response)
                                                        {
                                                            if([[response valueForKey:@"success"] intValue]==1)
                                                            {
                                                                ...
                                                                NSLog(@"Exact Controller - %@",self.navigationController);
...
//        [self.revealViewController.navigationController performSegueWithIdentifier:@"SegueToWalletVC" sender:self];
        return;
    }
    ...
    if ([[arrListName objectAtIndex:indexPath.row]isEqualToString:NSLocalizedStringFromTable(@"Share", [pref objectForKey:@"TranslationDocumentName"],nil)])
    {
        ...
        NSString *texttoshare = [NSString stringWithFormat:@"I am using JayeenTaxi Driver App ! Why don't you try it out... Install JayeenTaxi Driver now ! "]; //this is your text string to share
        ...
        [afn getDataFromPath:FILE_GET_REQUEST withParamData:dictparam withBlock:^(id response, NSError *error)
         {
             ...
             if (response)
             {
                 ...
                 //                 else segurtoarrived
                 //                 {
                 //                     pref=[NSUserDefaults standardUserDefaults]; segurtoarrived
